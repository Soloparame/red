...
const { result, loading, error } = useQuery(EPISODE_DETAILS_QUERY, { id });
...
<template>
  <div class="bg-[#3b2d2d] text-white min-h-screen flex flex-col">
    ...
    <main class="p-8 flex-grow flex flex-col items-center">
      ...
      <div v-if="error" class="text-center text-red-500">Error loading episode details</div>
      <div v-if="result && result.episode" class="max-w-2xl mx-auto bg-gray-800 p-6 rounded-lg shadow-lg border-2 border-red-500 ">
        <h1 class="text-4xl font-bold text-center mt-4 mb-6">{{ result.episode.name }}</h1>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <p class="text-lg font-semibold">Air Date:</p>
            <p class="text-white">{{ result.episode.air_date }}</p>
          </div>
          <div>
            <p class="text-lg font-semibold">Episode Code:</p>
            <p class="text-white">{{ result.episode.episode }}</p>
          </div>
        </div>
        <h3 class="text-2xl font-bold mt-8 mb-4">Characters</h3>
        <ul class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <li v-for="character in result.episode.characters" :key="character.id" class="flex flex-col items-center">
            <router-link :to="'/characters/' + character.id" class="inline-block relative"><img :src="character.image" alt="" class="rounded-full mb-2 mx-auto w-24 h-24 object-cover border-2 border-red-500 hover:scale-110 transition-transform duration-200" /></router-link>
